...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    features = util.Counter()
    #Define width constant for digit
    wRange = range(DIGIT_DATUM_WIDTH)
    #Define height constant for digit
    hRange = range(DIGIT_DATUM_HEIGHT)
    #Initialize the count to 0
    count = 0
    #Loop for all x in width of digit datum
    for x in wRange:
    #Loop for all y in height of digit datum
        for y in hRange:
            #Define a constant for getting pixels of digit
            getPix = datum.getPixel(x,y)
            #If datum is white
            if getPix > 0:
                features[(x,y)] = 1
            #Else datum is gray/black  
            else:
                features[(x,y)] = 0
    #Increment the counter      
    count+=1      

    #Now we loop for extra binary features added
    #for enhanced features in digit data set
    #Loop for all x in width of digit datum
    for x in hRange:
    #Initialize the variables to 0
        ...
        for y in wRange:
            #Define a constant for getting pixels of digit
            k = datum.getPixel(x,y)
            #If k is greater than 0 or 1 then datum is white
            white = k > 0
            #If k is 0 then datum is grey/balck
            grey = k == 0
            #First condition on binary values in feature
            condFirst = feature == 0 and white
            #Aux condition on binary values in features
            auxCond = feature == 1 and grey
            #Encode first feature that takes three binary values
            featFirst = pix == 0 and auxCond
            #Second condition on binary values in feature
            condSecond = feature == 1 and pix == 1 and white
            #Encode second feature that takes three binary values
            featSecond = pix == 1 and otherFeat == 1 and auxCond
            #Swtich the values of binary feature on/off as per
            #each condition
            if condFirst:
                feature = 1
            if featFirst:
            ...
            #Check if freq is greater than 0  
            if freqPix > 0:
                features[count] = 1
            else:
                features[count] = 0
            #Increment counter    
            count+=1
    #Return the feature as util.Counter()        
    return features
